...
| 缺口补数 | 最近交易日缺 K 线时（任务被延迟/漏跑）用腾讯逐只原始日线补齐，一次最多 3 天，成交额按均价估算并在 warnings 标注；dump 更新后自动覆盖 | —（本身逐只拉全量） |
...
之后每个工作日北京时间 18:40 自动运行（21:30 再跑一次兜底：数据未就绪时重试，已有结果则跳过）。每次运行还会自动补跑最近缺失的日报（最多 7 个交易日），被 GitHub 延迟触发或漏跑后能自愈；次日 11:10 还有一次早间兜底（dump 通常在 T+1 上午更新），补出前一日缺失的结果。节假日自动跳过。
